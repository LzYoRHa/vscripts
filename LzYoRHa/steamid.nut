...
//**********************************************************\\
//steam id 	stuff											\\
//Main logic from Darnias									\\
//put at csgo\scripts\vscripts\LzYoRHa						\\
//**********************************************************\\

::MySteamID <- "STEAM_1:0:191848162";
::MyUserID <- null;

::OnGameEvent_player_connect_raw<-function(event){
    if(event.networkid == ::MySteamID){
        ::MyUserID <- event.userid;;
        EntFire("IDBrush_LzYoRHa", "AddOutput","health " + ::MyUserID, 0, null);    
    }
}

function CheckID()
{
    local brushID = self.GetHealth();
	local ply = ::GetPlayerForUserID(brushID);
	print(brushID)
    if(ply != null)
    {
